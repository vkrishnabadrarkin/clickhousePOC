...
def generate_random_event(mins) -> dict:
    ...
    dst_VM_num = (((god_number//10000) + src_VM_num + 1) % 10000)
    ...
    return_value = {
        "name": flow_name,
        "time": int(event_datetime.timestamp()),
        "modelkey_cid": 12345,
        "modelkey_otype": 515,
...
async def write_to_event(data,csv_file):
    ...
    print(f"writing to {csv_file} {len(data)} rows; total count: {total_inserted_events}")


async def fill_events(_loop):
    for j in range(1,1441):
        s = time.time()
        csv_file = f"data_{j}_Min.csv"
        minutes = j
        for i in range(10):
            data = generate_random_events(inserter_config.BULK_SIZE,minutes)
            await write_to_event(data,csv_file)
        e = time.time()
        t = e-s
        log_experiment(t, csv_file)

def log_experiment(_took,csv_file):
    with open("csv_timeWise_permin_log.txt", 'a') as f:
