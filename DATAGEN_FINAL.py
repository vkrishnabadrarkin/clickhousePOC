            ...
            if (j < (h * s) / t):
                ...
                for l in range(t):
                    ...
                    if (total_inserted_events / s == total_inserted_events // s):
                        ...
                        df.to_csv(f'flow_Data_{i+1}_{h}original.csv.gz', header=False, index=False, compression='gzip')
                        events = events[s:]
    if (len(events) < s and len(events) > 0):
        df = pd.DataFrame(events)
        df.to_csv(f'remaining_data_versions{i}.csv.gz', header=False, index=False, compression='gzip')
    end = time.time()
    took = end - start
    with open(f'log_experiment{i}.txt', 'a') as logfile:
